-- Corrigir função parse_content_name para extrair episódios com mais dígitos
...
BEGIN
  ...
  -- Padrão 1: "Nome S01 E10" ou "Nome S01E10" (até 4 dígitos para episódio)
  IF working_name ~* '(.+?)\s*S(\d{1,2})\s*E(\d{1,4})' THEN
    match_result := regexp_match(working_name, '(.+?)\s*S(\d{1,2})\s*E(\d{1,4})', 'i');
    extracted_title := trim(match_result[1]);
    extracted_season := match_result[2]::integer;
    extracted_episode := match_result[3]::integer;
    
  -- Padrão 2: "Nome T01|EP10" ou "Nome T1|EP10" (até 4 dígitos)
  ELSIF working_name ~* '(.+?)\s*T(\d{1,2})\|EP(\d{1,4})' THEN
    match_result := regexp_match(working_name, '(.+?)\s*T(\d{1,2})\|EP(\d{1,4})', 'i');
    extracted_title := trim(match_result[1]);
    extracted_season := match_result[2]::integer;
    extracted_episode := match_result[3]::integer;

  -- Padrão 3: "T01|EP10" sem nome
  ELSIF working_name ~* '^T(\d{1,2})\|EP(\d{1,4})$' THEN
    match_result := regexp_match(working_name, '^T(\d{1,2})\|EP(\d{1,4})$', 'i');
    extracted_title := NULL;
    extracted_season := match_result[1]::integer;
    extracted_episode := match_result[2]::integer;

  -- Padrão 4: "Nome 1x03" ou "Nome 01x03"
  ELSIF working_name ~* '(.+?)\s+(\d{1,2})x(\d{1,4})' THEN
    match_result := regexp_match(working_name, '(.+?)\s+(\d{1,2})x(\d{1,4})', 'i');
    extracted_title := trim(match_result[1]);
    extracted_season := match_result[2]::integer;
    extracted_episode := match_result[3]::integer;
  ...
  -- Padrão 6: "Nome.S01.E01.720p"
  ELSIF working_name ~* '(.+?)\.S(\d{1,2})\.?E(\d{1,4})' THEN
    match_result := regexp_match(working_name, '(.+?)\.S(\d{1,2})\.?E(\d{1,4})', 'i');
